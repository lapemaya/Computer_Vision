...
def main(data_dir, model_name='yolov8n.pt', epochs=120, imgsz=640):
    ...
    results = model.train(
        ...
        # Ottimizzazione più aggressiva (mAP50-95 basso: 0.630)
        optimizer='AdamW',
        lr0=0.0015,  # Learning rate leggermente più alto
        lrf=0.01,
        momentum=0.937,
        weight_decay=0.0005,
        warmup_epochs=5.0,

        # Data augmentation più intensa per migliorare generalizzazione
        hsv_h=0.02,
        hsv_s=0.8,
        hsv_v=0.5,
        degrees=20.0,  # Rotazione più aggressiva
        translate=0.15,
        scale=0.7,
        flipud=0.0,
        fliplr=0.5,
        mosaic=1.0,
        mixup=0.2,  # Mixup più aggressivo

        # Early stopping più lungo
        patience=50,
        save_period=10,
        val=True,
        plots=True
    ...
    print(f"💾 Best weights: ../runs/train/yolo_Glasses/weights/best.pt")
    print(f"💾 Last weights: ../runs/train/yolo_Glasses/weights/last.pt")
    ...
    print(f"🎯 Usa il modello: ../runs/train/yolo_Glasses/weights/best.pt")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train YOLO on datasetGlasses folder")
    parser.add_argument('--data_dir', type=str, default='datasetGlasses', help='Path to dataset folder')
